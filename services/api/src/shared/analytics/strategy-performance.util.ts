...
export function calculateStrategyPerformance(
  ...
  for (const trade of trades) {
    if (!trade.strategy || !trade.strategyId) continue;

    let stats = strategyMap.get(trade.strategyId);
    ...
    if (!stats) {
      ...
      strategyMap.set(trade.strategyId, stats);
    }
  ...
  return Array.from(strategyMap.values()).map((s) => {
    ...
    const averageWin =
      s.wins === 0
        ? 0
        : s.grossProfit / s.wins;

    const averageLoss =
      s.losses === 0
        ? 0
        : s.grossLoss / s.losses;

    const profitFactor =
      s.grossLoss === 0
        ? 0
        : s.grossProfit / Math.abs(s.grossLoss);
